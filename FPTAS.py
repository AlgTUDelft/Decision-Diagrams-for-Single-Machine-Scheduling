...
class Solver:
                            ...
                            if not self.is_dominated(job, job_state, path_point_time, path_point, window):
                                for successor in job.successors[window.start_time]:
                                    ...
                                    if successor.id not in job_state.job_ids:
                                        self.try_path(job, successor, path_point_time, path_point, job_state, window, self.slack(epsilon))
                                ...
                                if path_point.value == max_value:
                                    for successor in job.far_offs[window.start_time]:
                                        if time.time()-t>1800:
                                            ...
                                            break
                                        if successor.id not in job_state.job_ids:
                                            self.try_path(job, successor, max_path_time, max_path, job_state, window, self.slack(epsilon))
                            #else:
    ...
    def try_path(self, job, successor, path_point_time, path_point, job_state, original_window, slack): 
        ...
        new_point =  PathPoint(value, successor, path_point, set(),partial_job_com_ids)
...
class PathPoint:
    __slots__ = ('value', 'job', 'previous', 'must_visit', 'upper','VisitedStates')
...
for R in [9]:
    for w in [11,13,15,17,19]:
         for ins in range(0, 10):
             ...
             t = -time.time()
             path = solver.solve_fptas(0.1)
             t += time.time()

             #all_t.append(t)
             try:
                 print("Width{0},{1},{2},{3}\t{4}\t{5}".format(n, w, R, ins, path.value, t), file=f)
                 print("Width{0},{1},{2},{3}\t{4}\t{5}".format(n, w, R, ins, path.value, t))
             except:
